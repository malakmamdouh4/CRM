     ...
     public function store(CreateFeature $request)
     {
        ...
        if (count($request->featuresIcon) > 0) {
            for ($i=0; $i<count($request->featuresIcon);$i++) {
                if ($request['featuresIcon'][$i] != '' && $request['featuresTitle'][$i] != '' && $request['featuresDescription'][$i] != '') {
                    $arr[] = [
                        'icon' => $request['featuresIcon'][$i],
                        'title' => $request['featuresTitle'][$i],
                        'description' => $request['featuresDescription'][$i]
                    ];
                }
            }
        }
        
        $data = $request->except(['_token','featuresIcon','featuresTitle','featuresDescription']);
